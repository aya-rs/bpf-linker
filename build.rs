...
fn link_llvm_static(stdout: &mut io::StdoutLock<'_>, llvm_lib_dir: &Path) -> anyhow::Result<()> {
    const LIBLLVM_PREFIX: &str = "LIBLLVM_PREFIX";
    writeln!(stdout, "cargo:rerun-if-env-changed={LIBLLVM_PREFIX}")?;
    let libllvm_prefix = env::var_os(LIBLLVM_PREFIX).unwrap_or_else(|| OsString::from("libLLVM"));
...
fn link_llvm_dynamic(stdout: &mut io::StdoutLock<'_>, llvm_lib_dir: &Path) -> anyhow::Result<()> {
    ...
    // found through `PATH`.
    //
    // Using rpath keeps that override scoped to the `bpf-linker` binaries,
    // unlike global linker configuration or `DYLD_LIBRARY_PATH` /
    // `LD_LIBRARY_PATH` which could also affect `rustc` on targets where it
    // links LLVM dynamically. That makes rpath the only isolation mechanism
    // suitable for `cargo run` and `cargo test`.
    write_bytes!(
        stdout,
...
fn main() -> anyhow::Result<()> {
    ...
    let (target_bits, lib_dir_candidates) = match target_address_size {
        ...
        target_address_size => anyhow::bail!(
