...
impl Display for Cxxstdlibs<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut libs = self.iter().peekable();
        while let Some(lib) = libs.next() {
            write!(f, "{}", OsStr::from_bytes(lib).display())?;
            if libs.peek().is_some() {
                write!(f, ", ")?;
            }
        }
        Ok(())
    ...
    if !cxxstdlib_found || (needs_zlib && !zlib_found) || (needs_zstd && !zstd_found) {
        ...
        fn print_search_dirs<'a>(
            ...
            if linker_output.status.success() {
                ...
                // We don't return error here, instead we log a warning and
...
{var}: {}) failed: {}",
        ...
        let (cc, linker_stdout) = [
            ...
            // overwrite defaults.
            (Some(CC), env::var_os(CC).map(Cow::Owned)),
            ...
            (
                Some(RUSTC_LINKER),
                env::var_os(RUSTC_LINKER).map(Cow::Owned),
            ),
            ...
            (None, Some(Cow::Borrowed(OsStr::from_bytes(b"cc")))),
        ]
        .into_iter()
        .find_map(|(var, maybe_cc)| {
            maybe_cc.and_then(|maybe_cc| print_search_dirs(stdout, var, maybe_cc).transpose())
        })
        .transpose()?
        .ok_or_else(|| {
            anyhow!(
...
to an appropriate compiler"
        ...
        for ld_path in env::split_paths(ld_paths) {
            ...
            if needs_zlib && let Some(ref mut zlib_paths) = zlib_paths {
            ...
            if needs_zstd && let Some(ref mut zstd_paths) = zstd_paths {
