...
Bazel can build bpf-linker and its LLVM dependency in a single graph without
requiring a separately installed LLVM. It uses the pinned Rust toolchain and
builds the pinned LLVM revision from source:
...
The archive contains FileCheck and LLVM's static component libraries. The macOS
and Linux archives also contain the shared LLVM library; Windows archives do
not, because their monolithic DLL exceeds the PE/COFF export limit. Windows
users must enable `llvm-link-static` and configure the static link environment
shown below. The archive is intended for building and debugging bpf-linker,
...
If `LLVM_PREFIX` is not set, `build.rs` searches `PATH` for `llvm-config` and
