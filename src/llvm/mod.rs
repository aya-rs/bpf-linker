    ...
    for function in module.functions_iter() {
        ...
        if !name.starts_with("llvm.") {
            ...
            if LLVMIsDeclaration(function) == 0 {
                let _modified = LLVMRunFunctionPassManager(fpm, function) != 0;
            }
        }
    }
    let _modified = LLVMFinalizeFunctionPassManager(fpm) != 0;

    debug!("running module passes");
    let _modified = LLVMRunPassManager(mpm, module) != 0;
