...
impl DIFile<'_> {
    ...
    pub(crate) fn filename(&self) -> Option<&CStr> {
        ...
        NonNull::new(ptr.cast_mut()).map(|ptr| unsafe { CStr::from_ptr(ptr.as_ptr()) })
    }
}
...
impl DICompositeType<'_> {
    ...
    #[expect(clippy::cast_sign_loss, reason = "replace as u32 with cast_unsigned when we no longer support LLVM 19")]
    pub(crate) fn elements(&self) -> impl Iterator<Item = Metadata<'_>> {
        ...
        (0..operands).map(move |i| unsafe {
            Metadata::from_value_ref(LLVMGetOperand(elements, i as u32))
        })
    }
...
impl DISubprogram<'_> {
    ...
    pub(crate) fn linkage_name(&self) -> Option<&str> {
        let operand =
