...
run_in_sysroot() {
  ...
  # To avoid hardcoding a specific version, pick the highest available one
  # (using version sort) so the selection is deterministic.
...
exec_compiler() {
  ...
  # architectures without additional linker wrappers.
  #
