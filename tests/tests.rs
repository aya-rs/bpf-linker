...
fn find_binary(binary_re_str: &str) -> PathBuf {
    ...
    let binary = binary.next();
    assert_ne!(binary, None, "Could not find {binary_re_str}");
    binary.unwrap()
}
...
fn compile_test() {
    ...
    let current_dir =
        env::var("CARGO_MANIFEST_DIR").expect("could not determine current directory");
    ...
    build_bitcode(
        current_dir.join("tests/c"),
        current_dir.join("target/bitcode"),
    );
