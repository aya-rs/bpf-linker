...
fn bpf_sysroot(root_dir: &Path) -> Option<PathBuf> {
...
fn bpf_sysroot(_root_dir: &Path) -> Option<PathBuf> {
    None
}
...
fn is_nightly() -> bool {
    ...
    if !output.status.success() {
        panic!(
            "failed to determine rustc version; :\n{}",
            String::from_utf8_lossy(&output.stderr).trim_end()
        );
    }
    let version = std::str::from_utf8(&output.stdout).expect("rustc version is not valid UTF-8");
...
fn compile_test() {
    ...
    let bpf_sysroot = bpf_sysroot(root_dir);
