...
#[derive(clap::Parser)]
struct DownloadLlvmSources {
    /// GitHub repository to download the sources from.
    #[arg(long, default_value = "aya-rs/llvm-project")]
    git_repo: String,
    /// Git ref (branch, commit, tag) to fetch.
    #[arg(long, default_value = "rustc/21.1-2025-08-01")]
    git_ref: String,
    /// Directory to extract the sources to.
    #[arg(long)]
    src_dir: PathBuf,
}
...
enum LlvmSubcommand {
    /// Downloads LLVM sources from GitHub.
    DownloadSources(DownloadLlvmSources),
    /// Builds LLVM.
    Build(BuildLlvm),
}
...
fn unpack_llvm_sources(src_dir: &Path, data: Vec<u8>) -> Result<()> {
    ...
    for entry_result in archive.entries()? {
        ...
        if let Some(stripped_path) = path.components().skip(1).collect::<PathBuf>().into() {
            if stripped_path.as_os_str().is_empty() {
                continue;
            }

            let dest_path = src_dir.join(stripped_path);
            let _unpacked = entry.unpack(dest_path)?;
        }
    }
...
fn download_llvm_sources(options: DownloadLlvmSources) -> Result<()> {
    ...
    let url = format!("https://github.com/{git_repo}/archive/refs/heads/{git_ref}.tar.gz");
    ...
    unpack_llvm_sources(&src_dir, data)
}
...
fn move_absolute_symlink_targets(install_prefix: &Path) -> Result<()> {
...
fn build_llvm(options: BuildLlvm) -> Result<()> {
    ...
    move_absolute_symlink_targets(&install_prefix)?;

    Ok(())
}

fn llvm(options: Llvm) -> Result<()> {
    let Llvm { subcommand } = options;
    match subcommand {
        LlvmSubcommand::DownloadSources(options) => download_llvm_sources(options),
        LlvmSubcommand::Build(options) => build_llvm(options),
    }
}
...
fn main() -> Result<()> {
    ...
    match subcommand {
        ...
        XtaskSubcommand::Llvm(options) => llvm(options),
    }
}
