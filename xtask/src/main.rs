...
fn get_llvm_version(toolchain: Option<&str>) -> Result<String> {
    ...
    let cmd_for_error = format!("{:?}", rustc_cmd);
    let output = rustc_cmd
        .output()
        .with_context(|| format!("failed to run {cmd_for_error}"))?;

    if !output.status.success() {
        return Err(anyhow!(
            "{cmd_for_error} failed with status {}",
            output.status
        ));
    }
    ...
    Err(anyhow!(
        "could not find `LLVM version:` line in {cmd_for_error} output"
    ))
}
...
fn rustc_llvm_commit(options: RustcLlvmCommitOptions) -> Result<()> {
    ...
    let llvm_version = get_llvm_version(toolchain.as_deref())?;
